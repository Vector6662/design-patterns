...
public class WaitNotifyQueue<T> {
    ...
    public synchronized void put(T resource) throws InterruptedException {
        ...
        // 是生产者线程，然后看到队列满了，再次休眠，再次随机唤醒一个线程...然后又有可能是生产者线程😅。
        // 这就可能造成一种”软死锁“（自己创造的名词），即一直唤醒生产者线程，这种情况概率很小但还是存在的。
        // 但即使采用notifyAll()，这种概率还是有的哦！notifyAll()唤醒所有的线程，让他们竞争锁资源。即使唤醒后又是生产者抢到了锁，
